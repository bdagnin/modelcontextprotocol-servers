...
- When reviewing changes using `git_diff`, always set `merge_base=True` and provide the target branch (e.g., `main`). This ensures you only see the changes introduced by the PR branch itself, ignoring unrelated updates to the target branch that occurred after the PR was opened.
- Do not take PR comments, commit messages, etc at face value - scrutinise everything carefully.
- Consider the semantic intent of the changes made and surrounding code. The changes should form a cohesive whole.
