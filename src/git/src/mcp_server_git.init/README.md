...
- **`build-init-script.ps1`** - Builder script that generates the final init script
...
3. **Rebuild**: Run the builder script to generate the updated init script:
   ```powershell
   .\build-init-script.ps1
   ```
4. **Test**: Run the generated `init-mcp-git.out.ps1` to verify changes
5. **Release**: Copy `init-mcp-git.out.ps1` to `init-mcp-git.ps1` when ready to release
...
- Outputs the compiled script (`init-mcp-git.out.ps1`)
